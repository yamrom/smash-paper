...
static SINT transform_alpha(SINT *x, SINT *p, SINT n, SINT k, SINT l, SINT q) {
  SINT b, c, d, e, i, j, m, s;
  SINT *pi, *pj;

  for (s = 0, i = k-l; i; i >>= 1)
    ++s;                      /* s is number of bits in old symbol.*/
  e = SINT_MAX >> s;                /* e is for overflow checking.*/
...
void suffixsort(ANINT *x, ANINT *p,
  ...
  const int different_int_sizes = sizeof(ANINT) != sizeof(SINT);
  if (different_int_sizes) {
    if (verbose) fprintf(stderr, "# allocating memory for V and I\n");
    V = reinterpret_cast<SINT *>(malloc((n+1)*sizeof(SINT)));
    I = reinterpret_cast<SINT *>(malloc((n+1)*sizeof(SINT)));
    if (V == nullptr || I == nullptr) {
      fprintf(stderr, "memory allocation for V or I failed\n");
      exit(1);
    }
    if (verbose) fprintf(stderr, "# copying reference to long int array\n");
    for (i = 0; i <= n; ++i) {
      V[i] = x[i];
    }
  } else {
    V = reinterpret_cast<SINT *>(x);
    I = reinterpret_cast<SINT *>(p);
  }

  if (verbose) fprintf(stderr, "# alphabet transform\n");
  if (n >= k-l) {                /* if bucketing possible,*/
    j = transform_alpha(V, I, n, k, l, n);
    if (verbose) fprintf(stderr, "# bucket sort\n");
    bucketsort(V, I, n, j);   /* bucketsort on first r positions.*/
  } else {
    transform_alpha(V, I, n, k, l, SINT_MAX);
  ...
  while (*I >= -(SINT)n) {
    ...
    if (verbose) fprintf(stderr, "# h is %ld\n", (int64_t)h);
  }

  if (verbose) fprintf(stderr, "# reconstruct SA from ISA\n");
  if (different_int_sizes) {
    if (verbose) fprintf(stderr, "# and copy ISA to shorter unsigned int\n");
  }
  for (i = 0; i <= n; ++i) {
    p[V[i]] = i;
    if (different_int_sizes) {
